...
if __name__ == '__main__':
    ...
    for epoch in range(args.num_epochs):
        ...
        avg_G_fake_loss = 0
        avg_kld = 0
        for i, (img, desc, len_desc, img_vggnorm) in enumerate(train_loader):
            #print i
            ...
            #print(i)
            img_vggnorm = Variable(img_vggnorm.cuda() if not args.no_cuda else img_vggnorm)
            img_G = img_vggnorm if args.use_vgg else img_norm
            #for i in range(img.size(0)):
                #img_G[i] = vgg_normalize(img.data[i])
